...
function AppHeader({
  ...
  const logoUrl =
    'https://lh3.googleusercontent.com/d/1EhYcDVJc8jezBSiGS1jJ6XM0EXxjvFKJ';
    ...
    <footer className="bg-[#09132B] text-white py-6 px-4 sm:px-8 border-t border-blue-900/40 text-center web-ui-only mt-auto">
      <div className="max-w-6xl mx-auto space-y-2 text-xs">
        <div className="font-extrabold text-sm sm:text-base text-amber-400 uppercase tracking-wide">
          PHÂN HIỆU HỌC VIỆN HÀNH CHÍNH VÀ QUẢN TRỊ CÔNG TẠI THÀNH PHỐ HỒ CHÍ
          MINH
        </div>
        <p className="text-gray-300 text-[11px] sm:text-xs">
          Cơ sở chính: Số 10, đường 3/2, Phường Hòa Hưng, Quận 10, TP. Hồ Chí
          Minh • Hotline KTX:{' '}
          <strong className="text-amber-300 font-mono">{hotline}</strong>
        </p>
...
export default function StudentDashboardPage() {
  ...
  const [activeScanTarget, setActiveScanTarget] = useState<'VNEID' | 'KTX'>(
    'VNEID'
  );
  ...
  // State Form thông tin tổng hợp (Để trống các trường tùy chọn để hiển thị placeholder mờ)
  ...
  const loadAllMasterData = async () => {
    try {
      ...
      const [
        ...
        supabase
          .from('danh_muc_quoc_gia')
          .select('ten_quoc_gia')
          .order('thu_tu', { ascending: true }),
        supabase
          .from('danh_muc_dan_toc')
          .select('ten_dan_toc')
          .order('thu_tu', { ascending: true }),
        supabase
      ...
      setQuocGiaList(
        qgRes.data
          ? qgRes.data.map((q) => q.ten_quoc_gia)
          : ['Việt Nam', 'Lào', 'Campuchia', 'Khác']
      );
      setDanTocList(
        dtRes.data
          ? dtRes.data.map((d) => d.ten_dan_toc)
          : ['Kinh', 'Tày', 'Thái', 'Mường', 'Khác']
      );
  ...
  const loadStudentInfo = async (cccd: string) => {
    ...
    try {
      ...
      if (st.ho_khau_thuong_tru) {
        const parts = st.ho_khau_thuong_tru
          .split(',')
          .map((p: string) => p.trim());
        if (parts.length >= 3) {
      ...
      const { data: bhyt } = await supabase
        .from('dang_ky_bhyt')
        .select('*')
        .eq('cccd', cccd)
        .eq('is_deleted', false)
      ...
      setFormData({
        ...
        diem_xet_tuyen:
          st.diem_xet_tuyen !== null && st.diem_xet_tuyen !== undefined
            ? Number(st.diem_xet_tuyen)
            : '',
  ...
  const filteredPhuongXaTT = useMemo(() => {
    if (!formData.tinh_thanh_tt)
      return diaChiList.slice(0, 100).map((d) => d.ten_xa);
    const list = diaChiList.filter(
      (d) => d.ten_tinh.toLowerCase() === formData.tinh_thanh_tt.toLowerCase()
    );
    return list.map((d) => d.ten_xa).sort();
  }, [diaChiList, formData.tinh_thanh_tt]);

  const filteredPhuongXaTamTru = useMemo(() => {
    if (!formData.tinh_thanh_tam_tru)
      return diaChiList.slice(0, 100).map((d) => d.ten_xa);
    const list = diaChiList.filter(
      (d) =>
        d.ten_tinh.toLowerCase() === formData.tinh_thanh_tam_tru.toLowerCase()
    );
    return list.map((d) => d.ten_xa).sort();
  }, [diaChiList, formData.tinh_thanh_tam_tru]);

  const autoCleanDetailAddress = (
    field: 'dia_chi_chi_tiet_tt' | 'dia_chi_chi_tiet_tam_tru',
    wardVal: string,
    provVal: string
  ) => {
    ...
    cleaned = cleaned.replace(
      /(,\s*|\s+)(phường|xã|thị trấn|quận|huyện|thị xã|thành phố|tỉnh|tp)\b.*$/gi,
      ''
    );
  ...
  const handleNoiCapChange = (type: string) => {
    ...
    if (type === 'KHAC') {
      ...
      const val = customNoiCap
        ? `Công an ${customNoiCap}`
        : formData.tinh_thanh_tt
        ? `Công an ${formData.tinh_thanh_tt}`
  ...
  const openScanner = async (target: 'VNEID' | 'KTX') => {
    ...
    try {
      ...
      alert(
        'Không thể mở Camera: ' +
          err.message +
          '. Bạn có thể chọn tải ảnh/PDF từ thiết bị.'
      );
      closeScanner();
    }
  };
      ...
      if (exportFormat === 'PDF') {
        if (!(window as any).jspdf) {
          await new Promise<void>((resolve, reject) => {
            ...
            script.onerror = () =>
              reject(new Error('Không tải được bộ xuất PDF'));
        ...
        const pdfFile = new File(
          [pdfBlob],
          `${prefix}_${studentData?.cccd || 'temp'}_${Date.now()}.pdf`,
        ...
        for (let i = 0; i < scannedPages.length; i++) {
          ...
          const imgFile = new File(
            [blob],
            `${prefix}_trang${i + 1}_${Date.now()}.jpg`,
            { type: 'image/jpeg' }
          );
  ...
  const uploadSingleFile = async (file: File, target: 'VNEID' | 'KTX') => {
    ...
    if (target === 'VNEID') {
      setFormData((prev) => ({
        ...prev,
        anh_minh_chung_vneid_urls: [
          ...prev.anh_minh_chung_vneid_urls,
          finalUrl,
        ],
      }));
    } else {
  ...
  const handleRemoveFile = (target: 'VNEID' | 'KTX', indexToRemove: number) => {
    if (target === 'VNEID') {
      setFormData((prev) => ({
        ...prev,
        anh_minh_chung_vneid_urls: prev.anh_minh_chung_vneid_urls.filter(
      ...
      setFormData((prev) => ({
        ...prev,
        minh_chung_ktx_urls: prev.minh_chung_ktx_urls.filter(
          (_, idx) => idx !== indexToRemove
        ),
      }));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const fullAddressTT = [
        formData.dia_chi_chi_tiet_tt,
        formData.phuong_xa_tt,
        formData.tinh_thanh_tt,
      ]
        .filter(Boolean)
        .join(', ');
      const fullAddressTamTru = formData.co_tam_tru
        ? [
            formData.dia_chi_chi_tiet_tam_tru,
            formData.phuong_xa_tam_tru,
            formData.tinh_thanh_tam_tru,
          ]
            .filter(Boolean)
            .join(', ')
        : null;
      ...
      const vneidUrlsPayload =
        formData.anh_minh_chung_vneid_urls.length > 1
          ? JSON.stringify(formData.anh_minh_chung_vneid_urls)
          : formData.anh_minh_chung_vneid_urls[0] || null;

      const ktxUrlsPayload =
        formData.minh_chung_ktx_urls.length > 1
          ? JSON.stringify(formData.minh_chung_ktx_urls)
          : formData.minh_chung_ktx_urls[0] ||
            dormReg?.minh_chung_url ||
      ...
      if (formData.dang_ky_ktx_choice) {
        const maHoSo =
          dormReg?.ma_ho_so ||
          `KTX26-${String(Math.floor(1000 + Math.random() * 9000))}`;
        const selectedRoom = coSoKtxList.find(
          (r) => r.id_toa_nha === formData.id_toa_nha
        );
        const tenKhu = selectedRoom
          ? `${selectedRoom.ten_toa_nha} (${selectedRoom.loai_phong})`
          : formData.khu_ktx_dang_ky;

        const { error: ktxError } = await supabase.from('dang_ky_ktx').upsert(
          {
            ma_ho_so: maHoSo,
      ...
      const { error: bhytError } = await supabase.from('dang_ky_bhyt').upsert(
        {
          cccd: studentData.cccd,
      ...
      setMessage({
        type: 'success',
        text: '🎉 Hồ sơ sinh viên, thông tin KTX & BHYT đã được cập nhật thành công!',
      });
    ...
    <div className="min-h-screen bg-slate-100 text-gray-900 flex flex-col font-sans">
      ...
      <main className="max-w-6xl w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-6 flex-1 web-ui-only">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white p-6 sm:p-8 rounded-2xl border border-gray-200 shadow-sm space-y-6">
            <div className="border-b border-gray-100 pb-3">
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                Vui lòng kiểm tra và điền đầy đủ các mục có dấu (*) để hoàn tất
                hồ sơ
              </p>
            </div>
            ...
            <div className="space-y-4 text-xs font-medium text-gray-700">
              ...
              {/* NGÀNH TRÚNG TUYỂN & ĐIỂM XÉT TUYỂN (ĐƯA LÊN PHẦN 1) */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, nganh_hoc: e.target.value })
                    }
                    required
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        diem_xet_tuyen: parseFloat(e.target.value) || 0,
                      })
                    }
                    required
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    <option value="" disabled>
                      -- Chọn hoặc nhập nơi cấp --
                    </option>
                    <option value="Cục Cảnh sát quản lý hành chính về trật tự xã hội">
                      1. Cục Cảnh sát quản lý hành chính về trật tự xã hội (Thẻ
                      gắn chip từ 10/10/2018)
                    </option>
                    <option value="Cục Cảnh sát ĐKQL cư trú và DLQG về dân cư">
                      2. Cục Cảnh sát ĐKQL cư trú và DLQG về dân cư (Mã vạch
                      01/01/2016 - 10/10/2018)
                    </option>
                    <option value="Bộ Công an">3. Bộ Công an</option>
                    <option value="Công an tỉnh / thành phố">
                      4. Công an tỉnh / thành phố trực thuộc Trung ương
                    </option>
                    <option value="KHAC">
                      5. Khác (Sinh viên tự nhập theo mặt sau CCCD)
                    </option>
                  </select>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, sdt_ca_nhan: e.target.value })
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, sdt_gia_dinh: e.target.value })
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, email_sv: e.target.value })
                    }
              ...
              <div className="pt-3 border-t border-gray-100">
                <div className="font-bold text-xs uppercase tracking-wider text-blue-900 mb-2 flex items-center gap-1.5">
                  <span>📍</span> Hộ khẩu thường trú (Theo CCCD / VNeID){' '}
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
                  <div className="md:col-span-6 space-y-1">
                    <label className="block font-bold text-gray-700 mb-1.5 leading-tight">
                      Địa chỉ chi tiết (Thôn/Xóm/Số nhà/Tên đường){' '}
                      <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          dia_chi_chi_tiet_tt: e.target.value,
                        })
                      }
                      onBlur={() =>
                        autoCleanDetailAddress(
                          'dia_chi_chi_tiet_tt',
                          formData.phuong_xa_tt,
                          formData.tinh_thanh_tt
                        )
                    ...
                    <p className="text-[11px] text-amber-700 font-semibold flex items-center gap-1 pt-0.5">
                      <span>⚠️</span> Lưu ý: Không nhập tên Phường/Xã hay
              ...
              <div className="pt-3 border-t border-gray-100 space-y-3">
                <div>
                  <label className="block font-bold text-gray-700 mb-1.5">
                    Sinh viên có TẠM TRÚ tại Thành phố Hồ Chí Minh không?{' '}
                    <span className="text-red-500">*</span>
                  </label>
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        co_tam_tru: e.target.value === 'true',
                      })
                    }
                  ...
                  >
                    <option value="false">
                      Không (Thường trú tại tỉnh hoặc đang ở KTX)
                    </option>
                    <option value="true">
                      Có (Đang tạm trú / thuê trọ ngoài)
                    </option>
                  </select>
                </div>
                ...
                {formData.co_tam_tru && (
                  <div className="p-4 bg-blue-50/50 rounded-2xl border border-blue-200 space-y-3">
                    <div className="font-bold text-xs uppercase tracking-wider text-brand-blue flex items-center gap-1.5">
                      <span>🏠</span> Địa chỉ Tạm trú (Xác định trên VNeID){' '}
                      <span className="text-red-500">*</span>
                    </div>
                      ...
                      <div className="md:col-span-6 space-y-1">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              dia_chi_chi_tiet_tam_tru: e.target.value,
                            })
                          }
                          onBlur={() =>
                            autoCleanDetailAddress(
                      ...
                      <div className="md:col-span-3">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              tinh_thanh_tam_tru: e.target.value,
                              phuong_xa_tam_tru: '',
                            })
                          }
                          placeholder="Gõ tìm Tỉnh/Thành..."
                      ...
                      <div className="md:col-span-3">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              phuong_xa_tam_tru: e.target.value,
                            })
                          }
                          placeholder="Gõ tìm Phường/Xã..."
                          required={formData.co_tam_tru}
              ...
              <div className="pt-3 border-t border-gray-100 space-y-2.5">
                <label className="block font-bold text-gray-700">
                  Ảnh chụp / Minh chứng: Nơi thường trú, Nơi Tạm trú CCCD trên
                  VNeID <span className="text-red-500">*</span>
                </label>
                ...
                {formData.anh_minh_chung_vneid_urls.length > 0 ? (
                  <div className="space-y-2 pt-1">
                    {formData.anh_minh_chung_vneid_urls.map((url, idx) => (
                      <div
                        key={`vneid-file-${idx}`}
                        className="flex items-center justify-between p-2.5 bg-emerald-50 border border-emerald-200 rounded-xl text-xs"
                      >
                        <div className="flex items-center gap-2 overflow-hidden">
                          <span className="text-emerald-700 font-bold">
                            📄 Tệp {idx + 1}:
                          </span>
                          <span className="text-emerald-900 font-mono truncate max-w-xs sm:max-w-md">
                            {url}
                          </span>
                        </div>
                  ...
                  <p className="text-[11px] text-gray-500 flex items-center gap-1">
                    <span>💡</span> Bạn có thể chụp liên tiếp nhiều trang hoặc
                    chọn cùng lúc nhiều ảnh màn hình VNeID mức 2.
                  </p>
                )}
              </div>
            </div>
          ...
          {/* MỤC 2: ĐĂNG KÝ NGUYỆN VỌNG KTX (ĐÃ ĐỔI TÊN TIÊU ĐỀ) */}
          <div className="bg-white p-6 sm:p-8 rounded-2xl border border-gray-200 shadow-sm space-y-5">
              ...
              <label className="flex items-center gap-2 text-xs font-bold cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      dang_ky_ktx_choice: e.target.checked,
                    })
                  }
              ...
              <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div>
                  ...
                  <p className="text-xs text-gray-700 mt-1">
                    Khu vực đăng ký: <strong>{dormReg.khu_ktx_dang_ky}</strong>{' '}
                    • Bậc: {dormReg.bac_uu_tien}
                  </p>
                  {dormReg.ly_do_tu_choi && (
                    <p className="text-xs text-red-600 font-semibold mt-1">
                      Lý do từ chối: {dormReg.ly_do_tu_choi}
                    </p>
                  )}
                </div>
            ...
            {formData.dang_ky_ktx_choice && (
              <div className="space-y-4 pt-2 text-xs font-medium text-gray-700">
                <div>
                  <label className="block font-bold text-gray-700 mb-1.5">
                    Bậc ưu tiên xét duyệt KTX{' '}
                    <span className="text-red-500">*</span>
                  </label>
                  <select
                    value={formData.bac_uu_tien}
                    onChange={(e) =>
                      setFormData({ ...formData, bac_uu_tien: e.target.value })
                    }
                ...
                <div>
                  <label className="block font-bold text-gray-700 mb-1.5">
                    Khu Ký túc xá đăng ký{' '}
                    <span className="text-red-500">*</span>
                  </label>
                  <select
                    value={formData.id_toa_nha}
                    onChange={(e) => {
                      const selected = coSoKtxList.find(
                        (r) => r.id_toa_nha === e.target.value
                      );
                      setFormData({
                        ...formData,
                        id_toa_nha: e.target.value,
                        khu_ktx_dang_ky: selected
                          ? `${selected.ten_toa_nha} (${selected.loai_phong})`
                          : '',
                      });
                    }}
                    required={formData.dang_ky_ktx_choice}
                  ...
                  >
                    <option value="" disabled>
                      -- Chọn cơ sở KTX đăng ký --
                    </option>
                    {coSoKtxList.map((room) => (
                      <option key={room.id_toa_nha} value={room.id_toa_nha}>
                        {room.ten_toa_nha} ({room.loai_phong}) – Còn{' '}
                        {room.so_giuong_trong} chỗ
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div className="pt-2 border-t border-gray-100 space-y-2.5">
                  <label className="block font-bold text-gray-700">
                    Minh chứng Bậc ưu tiên KTX (Sổ hộ nghèo, con TB/LS, Tuyển
                    thẳng...)
                  </label>
                  ...
                  {formData.minh_chung_ktx_urls.length > 0 ? (
                    ...
                    <p className="text-[11px] text-gray-500">
                      💡 Sinh viên diện Bậc 1, 2, 3 vui lòng scan hoặc chụp tải
                      lên toàn bộ các trang giấy tờ minh chứng.
                    </p>
                  )}
                </div>
              </div>
          ...
          <div className="bg-white p-6 sm:p-8 rounded-2xl border border-gray-200 shadow-sm space-y-6">
            <div className="border-b border-gray-100 pb-3 flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  Phục vụ công tác cấp thẻ BHYT học sinh - sinh viên và quyền
                  lợi chăm sóc sức khỏe ban đầu
                </p>
              </div>
              {bhytReg && (
                <span
            ...
            <div className="space-y-4 text-xs font-medium text-gray-700">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block font-bold text-gray-700 mb-1.5">
                    Mã thẻ BHYT trên VssID hoặc VNeID (gồm cả chữ và số){' '}
                    <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block font-bold text-gray-700 mb-1.5">
                    Giá trị sử dụng thẻ BHYT hiện tại trên VssID hoặc VNeID{' '}
                    <span className="text-red-500">*</span>
                  </label>
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        han_su_dung_bhyt: e.target.value,
                      })
                    }
                    required
              ...
              <div>
                <label className="block font-bold text-gray-700 mb-1.5">
                  Đối tượng tham gia BHYT năm học 2026 - 2027{' '}
                  <span className="text-red-500">*</span>
                </label>
                <select
                  value={formData.doi_tuong_bhyt}
                  onChange={(e) =>
                    setFormData({ ...formData, doi_tuong_bhyt: e.target.value })
                  }
                  required
                  className="w-full px-4 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-[#0E1E45] focus:outline-none bg-white font-semibold cursor-pointer"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-1 items-end">
                <div>
                  <label className="block font-bold text-gray-700 mb-1.5 min-h-[36px] flex items-end leading-tight">
                    <span>
                      Khám SK theo KH số 228/KH-UBND của UBND TP.HCM tại địa
                      phương? <span className="text-red-500">*</span>
                    </span>
                  </label>
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        da_kham_sk_kh228: e.target.value,
                      })
                    }
                    required
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-[#0E1E45] focus:outline-none bg-white font-medium cursor-pointer"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, quoc_tich: e.target.value })
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, dan_toc: e.target.value })
                    }
          ...
          <div className="flex justify-end gap-3">
            ...
            >
              <span>💾</span>
              <span>
                {submitting ? 'Đang lưu hồ sơ...' : 'Lưu & Hoàn Thiện Hồ Sơ'}
              </span>
            </button>
          </div>
        </form>
      ...
      <AppFooter
        namHoc={systemConfigs.NAM_HOC}
        hotline={systemConfigs.HOTLINE_KTX}
      />
          ...
          <div className="bg-[#121212] text-white rounded-3xl max-w-lg w-full h-[90vh] max-h-[750px] flex flex-col overflow-hidden relative border border-white/10 shadow-2xl">
            <div className="p-4 flex justify-between items-center border-b border-white/10 bg-black/40">
              ...
              <div className="text-xs font-bold uppercase tracking-wider text-amber-400">
                {activeScanTarget === 'VNEID'
                  ? '📸 Scan Cư trú VNeID'
                  : '📸 Scan Minh chứng Ưu tiên KTX'}
                {scannedPages.length > 0 &&
                  ` (Đã chụp ${scannedPages.length} trang)`}
              </div>
              {scannedPages.length > 0 ? (
                <button
                  type="button"
                  ...
                  {scannedPages.length > 0 && (
                    <div className="flex items-center gap-1 bg-black/70 p-1 rounded-xl border border-white/20 overflow-x-auto max-w-[150px]">
                      {scannedPages.map((p, idx) => (
                        <div
                          key={idx}
                          className="relative w-8 h-10 rounded border border-blue-400 overflow-hidden shrink-0"
                        >
                          <img
                            src={p}
                            alt={`Trang ${idx + 1}`}
                            className="w-full h-full object-cover"
                          />
                          <span className="absolute bottom-0 right-0 bg-blue-600 text-[8px] px-1 font-bold">
                            {idx + 1}
                          </span>
                        </div>
                      ))}
      ...
      <div id="printA4Template">
        ...
        >
          ...
          <div style={{ textAlign: 'center', margin: '6px 0 8px 0' }}>
            ...
            >
              (Mã tiếp nhận trực tuyến:{' '}
              <strong style={{ fontSize: '11pt', color: 'black' }}>
                {dormReg?.ma_ho_so || 'KTX26-0024'}
              </strong>
              )
            </div>
          </div>
              ...
              <tbody>
                <tr>
                  ...
                  <td style={{ verticalAlign: 'top' }}>
                    - Ban Giám đốc Phân hiệu Học viện Hành chính và Quản trị
          ...
          >
              ...
              <tbody>
                <tr>
                  <td style={{ width: '50%' }}>
                    Mã sinh viên (MSSV):{' '}
                    <strong>
                      {formData.ma_sv || studentData.ma_sv || 'N/A'}
                    </strong>
                  </td>
                  <td style={{ width: '50%' }}>
                    Giới tính:{' '}
                    <span>{formData.gioi_tinh || studentData.gioi_tinh}</span>
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <div style={{ marginBottom: '3px' }}>
              ...
              <span>
                {formData.ngay_sinh
                  ? formData.ngay_sinh.split('-').reverse().join('/')
                  : ''}
              </span>
            </div>
            <div style={{ marginBottom: '3px' }}>
              ...
              <tbody>
                <tr>
                  ...
                  <td style={{ width: '25%' }}>
                    Ngày cấp:{' '}
                    <span>
                      {formData.ngay_cap_cccd
                        ? formData.ngay_cap_cccd.split('-').reverse().join('/')
                        : ''}
                    </span>
                  </td>
                  <td style={{ width: '30%' }}>
                    Nơi cấp: <span>{formData.noi_cap_cccd}</span>
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <div style={{ marginBottom: '3px' }}>
              ...
              <span>
                {[
                  ...
                  .filter(Boolean)
                  .join(', ') || studentData.ho_khau_thuong_tru}
              </span>
            </div>
              ...
              <tbody>
                <tr>
                  <td style={{ width: '50%' }}>
                    ...
                    <strong>
                      {formData.sdt_ca_nhan || studentData.sdt_ca_nhan}
                    </strong>
                  </td>
                  <td style={{ width: '50%' }}>
                    Số điện thoại gia đình:{' '}
                    <span>
                      {formData.sdt_gia_dinh || studentData.sdt_gia_dinh}
                    </span>
                  </td>
                </tr>
              </tbody>
            ...
            <div style={{ marginBottom: '3px' }}>
              Khu Ký túc xá đăng ký:{' '}
              <strong>
                {dormReg?.khu_ktx_dang_ky ||
                  formData.khu_ktx_dang_ky ||
                  'KTX 3 tầng (Số 10 đường 3/2)'}
              </strong>
            </div>
            <div style={{ marginBottom: '3px' }}>
              Bậc ưu tiên xét duyệt KTX:{' '}
              <span style={{ fontStyle: 'italic' }}>
                {dormReg?.bac_uu_tien || formData.bac_uu_tien}
              </span>
            </div>
            ...
            >
              Tôi làm đơn này được đăng ký ở tại Ký túc xá để tiện sinh hoạt và
              học tập. Tôi xin cam kết ở đúng số phòng - số giường đã được xếp,
              thực hiện nghiêm túc nội quy Ký túc xá, thanh toán đầy đủ các
              khoản phí và trả phòng đúng thời gian quy định. Em xin mang theo
              giấy tờ minh chứng diện ưu tiên bản chính/bản sao công chứng để
              nộp trực tiếp khi làm thủ tục nhập học.
            </p>
          ...
          >
            * Thí sinh in đơn này khổ A4 và nộp kèm bản cứng giấy tờ minh chứng
            diện ưu tiên khi làm thủ tục nhập học trực tiếp tại số 10 đường 3/2.
          </div>
        </div>
      </div>
    </div>
